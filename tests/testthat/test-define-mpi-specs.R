list_name <- c("indicators", "poverty_cutoffs", "uid", "unit_of_analysis", "source_of_data")
colname <- c("dimension", "indicator", "variable", "weight")

test_that("only accepts poverty cutoff between 0 and 1", {
  file <- '../../inst/extdata/global-mpi-specs.csv'
  expect_error(
    define_mpi_specs(file, .poverty_cutoffs = c(0, 1)),
  ...
  expect_error(
    define_mpi_specs(file, .poverty_cutoffs = c(-0.6, 0.1, 1/3)),
    '.poverty_cutoffs cannot contain values greater than 1.'
  )
})
...
test_that("[csv] sample specs file loads correctly", {
  ...
  expect_contains(names(mpi_specs), list_name)
...
test_that("[xlsx] sample specs file loads correctly", {
  file <- '../../inst/extdata/global-mpi-specs.xlsx'
...
test_that("[txt] sample specs file loads correctly", {
  file <- '../../inst/extdata/global-mpi-specs.txt'
...
test_that("[json] sample specs file loads correctly", {
  file <- '../../inst/extdata/global-mpi-specs.json'
