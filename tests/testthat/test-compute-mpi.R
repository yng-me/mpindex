...
test_that("compute_mpi accepts a pre-built mpi_deprivation_matrix directly", {
  dm <- df_household |>
    create_deprivation_matrix(deprivation_profile, .mpi_specs = mpi_specs)

  mpi_from_dm <- dm |> compute_mpi(.mpi_specs = mpi_specs)
...
test_that("contributions sum to 100 across each dimension for multi-cutoff", {
  mpi_specs_multi <- use_global_mpi_specs(
    .uid = "uuid",
    .poverty_cutoffs = c(1 / 3, 1 / 2)
  )
  mpi <- df_household |>
    compute_mpi(deprivation_profile, .mpi_specs = mpi_specs_multi)
