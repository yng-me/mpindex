...
First, we need to create an empty list, and name it `dp` (but feel free to name it whatever you like). 

```{r}
dp <- list()
```

To create a deprivation profile for each indicator, we use the `define_deprivation` function (see `?define_deprivation`) and add to the `dp` list we created above. Make sure that the deprivation profile for each indicator matches the variable name declared in the specification file.
...
```{r}
dp$child_mortality <- df_household |> 
  define_deprivation(
...
```{r}
dp$year_schooling <- df_household_roster |> 
  define_deprivation(
...
```{r}
dp$school_attendance <- df_household_roster |> 
  define_deprivation(
...
After completing the deprivation profile, use the `compute_mpi` function and pass the `dp` list as the first argument.
...
# ----------------------------------
# Compute the MPI
mpi_result <- compute_mpi(df_household, dp)
